...
function isExpired($couponId) {
    $coupon = Coupon::where('id', $couponId)->orderBy('id', 'desc')->first();

    if ($coupon && $coupon->expire_date >= now()) {
        $couponDate = Carbon::parse($coupon->expire_date)->format('d-m-Y');
        // Coupon is active
        return 'Expire at: '.$couponDate;
    } else {
        // Coupon is expired
